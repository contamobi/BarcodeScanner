...
- (zxing::Ref<zxing::LuminanceSource>) getLuminanceSourceFromSample:(CMSampleBufferRef)sampleBuffer imageBytes:(uint8_t**)ptr {
    ...
    size_t    greyHeight = MIN(width, height),
    greyWidth  = UIInterfaceOrientationIsLandscape(ori) ? MAX(width, height): greyHeight;
    ...
    *ptr = greyData;
    ...
    Ref<LuminanceSource> luminanceSource (
                                          new GreyscaleLuminanceSource(greyData, (int)greyHeight, (int)greyWidth, 0, 0, (int)greyHeight, (int)greyWidth, (cycle++ % 5 == 0))
                                          );
...
- (UIImage*)buildReticleImage {
    ...
    if (self.processor.is1D) {
        ...
        CGFloat lineOffset = 0;//RETICLE_OFFSET+(0.5*RETICLE_WIDTH);
